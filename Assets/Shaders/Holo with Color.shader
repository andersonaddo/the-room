...
Shader "HOLO/Holo with Color"
{
	Properties
	{
		...
		[HDR] _GlitchColor ("G/H Color", Color) = (1,1,1,1)
	...
	Subshader
	{
		...
		Pass
		{
			...
			half4 frag(vertexOutput i , float facing : VFACE) : COLOR
			{
				...
				//// Color distortion
				float4 c_2 = _Color;
				c_2.r += (temp * sin(T*1) * glitch_res.x * (_intensity)*0.25) *0.75 * _GlitchColor.r ;
				c_2.g +=(temp * sin(T*2) * glitch_res.y * (_intensity)*0.5)  *0.75 * _GlitchColor.g;
				c_2.b += (temp * sin(T*4)  * glitch_res.x * (_intensity)) *0.75 * _GlitchColor.b;
				...
				if (_monochrom == 1)
				{
					...
					c_2.rgb = float3(c_2_to_grey * _GlitchColor.r ,c_2_to_grey * _GlitchColor.g,c_2_to_grey * _GlitchColor.b);
	...
	CustomEditor "Glitch_Editor_lite_color"
}
